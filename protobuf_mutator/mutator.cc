...
using UnpackedAny = std::unordered_map<const Message*, std::unique_ptr<Message>>;
...
std::unique_ptr<Message> UnpackAny(const Any& any) {
  ...
  if (!desc) return {};
  std::unique_ptr<Message> message(any.GetReflection()->GetMessageFactory()->GetPrototype(desc)->New());
  message->ParsePartialFromString(std::string(any.value()));
  return message;
}

const Any* CastToAny(const Message* message) {
  return Any::GetDescriptor() == message->GetDescriptor() ? static_cast<const Any*>(message) : nullptr;
}

Any* CastToAny(Message* message) {
  return Any::GetDescriptor() == message->GetDescriptor() ? static_cast<Any*>(message) : nullptr;
}
...
class PostProcessing {
 public:
  ...
  PostProcessing(bool keep_initialized, const PostProcessors& post_processors,
      ...
      : keep_initialized_(keep_initialized), post_processors_(post_processors), any_(any), random_(random) {}
...
class FieldMutator {
 public:
  FieldMutator(int size_increase_hint, bool enforce_changes, bool enforce_utf8_strings, 
              const ConstMessages& sources, Mutator* mutator)
      : size_increase_hint_(size_increase_hint), enforce_changes_(enforce_changes),
        enforce_utf8_strings_(enforce_utf8_strings), sources_(sources), mutator_(mutator) {}
  ...
  void Mutate(std::string* value) const {
    if (enforce_utf8_strings_) 
      RepeatMutate(value, std::bind(&Mutator::MutateUtf8String, mutator_, _1, size_increase_hint_));
    else 
      RepeatMutate(value, std::bind(&Mutator::MutateString, mutator_, _1, size_increase_hint_));
  }
...
struct MutateField : public FieldFunction<MutateField> {
  ...
  void ForType(const FieldInstance& field, int size_increase_hint,
    ...
    T value;
    field.Load(&value);
    FieldMutator(size_increase_hint, true, field.EnforceUtf8(), sources, mutator).Mutate(&value);
    field.Store(value);
  }
};
...
struct CreateField : public FieldFunction<CreateField> {
 public:
  ...
  void ForType(const FieldInstance& field, int size_increase_hint,
    ...
    FieldMutator field_mutator(size_increase_hint, false, field.EnforceUtf8(), sources, mutator);
...
void Mutator::Fix(Message* message) {
  ...
  PostProcessing(keep_initialized_, post_processors_, any, &random_).Run(message, kMaxInitializeDepth);
...
void Mutator::CrossOver(const Message& message1, Message* message2, size_t max_size_hint) {
  ...
  PostProcessing(keep_initialized_, post_processors_, any, &random_).Run(message2, kMaxInitializeDepth);
...
std::string Mutator::MutateString(const std::string& value, int size_increase_hint) {
  ...
  while (size_increase_hint > 0 && result.size() < static_cast<size_t>(size_increase_hint) 
...
std::string Mutator::MutateUtf8String(const std::string& value, int size_increase_hint) {
