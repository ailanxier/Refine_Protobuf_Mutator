...
namespace {
  ...
  public:
    DataSourceSampler(const ConstFieldInstance& match, RandomEngine* random, int size_increase)
        : match_(match), random_(random), size_increase_(size_increase), sampler_(random) {}
  ...
  void Mutator::TryMutateField(Message* msg, FieldDescriptor* field, MutationBitset& allowed_mutations, int& remain_size){
    ...
    int order = GetRandomIndex(&random_, tot);
